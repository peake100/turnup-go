...
type patternPhaseAuto struct {
	...
	pricePeriodGen    *subPeriodsGenerator
	pricePeriodGenMax *subPeriodsGenerator
}
...
func (phase *patternPhaseAuto) setup(period PricePeriod, subPeriod int) {
	...
	phase.pricePeriodGen = &subPeriodsGenerator{
		Ticker:           phase.Ticker(),
	...
	if !phase.purchasePriceKnown {
		phase.pricePeriodGenMax = &subPeriodsGenerator{
			Ticker:           phase.Ticker(),
			PurchasePrice:    110,
			PhaseFull:        phase,
			PricePeriodStart: phaseStartPeriod,
...
func (phase *patternPhaseAuto) periodFromGen(
	subPeriod int,
