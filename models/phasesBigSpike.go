...
func (phase *sharpIncrease) BasePriceMultiplier(subPeriod int) (min float32, max float32) {
	switch {
	case subPeriod == 0:
		return 0.9, 1.4
	case subPeriod == 1:
...
func (phase *sharpDecrease) BasePriceMultiplier(subPeriod int) (min float32, max float32) {
	if subPeriod == 0 {
		return 1.4, 2
	}
